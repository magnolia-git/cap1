...
public class MeritBankController {
	...
	@PreAuthorize("hasAuthority('admin')")
	@ResponseStatus(HttpStatus.OK)
	@GetMapping(value = "/AccountHolders/{id}/Transactions")
	public List<Transaction> getTransactionsById(@PathVariable int id) throws AccountNotFoundException {
		return null;//meritBankService.getTransactionsById(id);
	}

	@PreAuthorize("hasAuthority('admin')")
	@PostMapping(value = "/AccountHolders/{id}/Deposit/{baID}")
	public Transaction postDeposit(@Valid @RequestBody DepositTransaction deposit, 
			@PathVariable("id") int id,@PathVariable("baID") int baID)
			throws AccountNotFoundException, ExceedsCombinedBalanceLimitException {
		return meritBankService.postDeposit(deposit, id, baID);
	}
}